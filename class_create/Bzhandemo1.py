...
def analysis_data(content):
    ...
    for i in list:
        ...
        message.append(name)
        message.append(i.get('short_link_v2'))#视频网页链接
...
def save_data(lists):
    exist=os.path.getsize('2023_hotDATA.csv')
    headLine = ['视频标题', '视频作者', '网页链接', '主题', '播放量', '点赞量', '硬币量', '收藏数']
    if exist:
        df = pd.DataFrame(lists)
        df.to_csv('2023_hotDATA.csv', mode='a', index=False, encoding='utf-8-sig', header=None)
    else:
        df=pd.DataFrame(lists,columns=headLine)
        df.to_csv('2023_hotDATA.csv', mode='a', index=False, encoding='utf-8-sig', errors='ignore')

if __name__=='__main__':
    with open("2023_hotDATA.csv", "w"):
        pass
